      ...
      <div class="card-header bg-white border-0 py-4 px-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
        <div>
          <h2 class="color_icon fw-bold mb-1">
            <i class="bi bi-journal-text me-2 color_icon"></i>Lista de Observador
          </h2>
          <p class="text-muted mb-0 small">Administra todas las observaciones registradas</p>
        </div>

        <!-- Botones de acción -->
        <div class="d-flex flex-wrap gap-2 mt-3 mt-md-0">
          <a href="Controlador.php?accion=modulos" class="btn btn-outline-secondary rounded-pill px-4 btn_modulos">
            <i class="bi bi-grid-3x3-gap-fill me-1"></i> Módulos
          </a>
          <a href="Controlador.php?accion=observador_insertar" class="btn btn-primary rounded-pill px-4 btn_app">
            <i class="bi bi-plus-circle me-1"></i> Agregar
          </a>
          <a href="Controlador.php?accion=observador_pdf" class="btn btn-danger rounded-pill px-4 btn_exportar">
      ...
      <div class="px-4 pb-3">
        <form action="buscar_usuario.php" method="get" class="w-100 w-md-50 ms-auto">
          <div class="input-group shadow-sm rounded-pill overflow-hidden">
            <input type="text" name="busqueda" id="busqueda" class="form-control shadow-sm border-0 ps-4" placeholder="Buscar">
          ...
          <table class="table align-middle table-hover mb-0">
            ...
            <tbody class="border-top">
              ...
              while ($d = $datos->fetch_object()) {
                  $data = '
                  <tr>
                      <td class="ps-4 fw-semibold text-dark">' . $d->codobservador . '</td>
                      <td>' . $d->idpersona . '</td>
                      <td class="text-capitalize">' . $d->pernombre . '</td>
                      <td class="text-capitalize">' . $d->perapellido . '</td>
                      <td>' . $d->obsfecha . '</td>
                      <td>' . $d->obsnumerobimestre . '</td>
                      <td>' . $d->obsobservacion . '</td>
                      <td>' . $d->tipfalnombre . '</td>
                      <td>' . $d->tipfaldescripcion . '</td>
                      <td class="text-center">
                          <div class="d-flex justify-content-center gap-2">
                              <a href="Controlador.php?accion=observador_consultar&id=' . $d->codobservador . '" class="btn btn-outline-primary action-btn" title="Ver">
                                  <i class="bi bi-eye"></i>
                              </a>
                              <a href="Controlador.php?accion=observador_actualizar&id=' . $d->codobservador . '" class="btn btn-outline-success action-btn" title="Editar">
                                  <i class="bi bi-pencil"></i>
                              </a>
                              <a href="Controlador.php?accion=observador_eliminar&id=' . $d->codobservador . '" class="btn btn-outline-danger action-btn" title="Eliminar" >
      ...
      <div class="card-footer bg-white py-4">
        <nav aria-label="Navegación de páginas">
          <ul class="pagination justify-content-center gap-2 mb-0">
            ...
            $anteriorLink = ($paginaActual > 1) ? 'Controlador.php?accion=modulo_observador&pagina=' . (($anterior - 1) * 10) : '#';
            ...
            for ($j = 1; $j <= $totalPaginas; $j++) {
                ...
                echo '<li class="page-item ' . $activeClass . '">
                        <a class="page-link pagination-link" href="Controlador.php?accion=modulo_observador&pagina=' . $c . '">' . $j . '</a>
                      </li>';
            }

            // Página siguiente
            $siguiente = min($totalPaginas, $paginaActual + 1);
            $siguienteLink = ($paginaActual < $totalPaginas) ? 'Controlador.php?accion=modulo_observador&pagina=' . (($siguiente - 1) * 10) : '#';
