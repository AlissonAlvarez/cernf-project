  ...
  <div class="container-fluid px-4">
    <div class="card shadow-lg border-0 rounded-4 overflow-hidden">
      ...
      <div class="card-header bg-white border-0 py-4 px-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
        <div>
          <h2 class="fw-bold mb-1 color_icon"><i class="bi bi-list-check me-2 color_icon"></i>Lista de Tipo de Faltas</h2>
          <p class="text-muted mb-0 small">Administra todos los tipos de faltas registrados en el sistema</p>
        </div>
        ...
        <div class="d-flex flex-wrap gap-2 mt-3 mt-md-0">
          ...
          <a href="Controlador.php?accion=tipofaltas_insertar" class="btn btn-primary rounded-pill px-4 btn_app">
            <i class="bi bi-plus-circle me-1"></i> Agregar
          </a>
          <a href="Controlador.php?accion=tipofaltas_pdf" class="btn btn-danger rounded-pill px-4 btn_exportar">
      ...
      <div class="px-4 pb-3">
        <form action="buscar_usuario.php" method="get" class="w-100 w-md-50 ms-auto">
          <div class="input-group shadow-sm rounded-pill overflow-hidden">
            <input type="text" name="busqueda" id="busqueda" class="form-control shadow-sm border-0 ps-4" placeholder="Buscar tipo de falta...">
          ...
          <table class="table align-middle table-hover mb-0">
            ...
            <tbody class="border-top">
              ...
              while ($d = $datos->fetch_object()) {
                $data = '
                  <tr>
                      <td class="ps-4 fw-semibold text-dark">' . $d->codtipofalta . '</td>
                      <td class="text-capitalize">' . $d->tipfalnombre . '</td>
                      <td>' . $d->tipfaldescripcion . '</td>
                      <td class="text-center">
                          <div class="d-flex justify-content-center gap-2">
                              <a href="Controlador.php?accion=tipofaltas_consultar&id=' . $d->codtipofalta . '" class="btn btn-outline-primary action-btn" title="Ver">
                                  <i class="bi bi-eye"></i>
                              </a>
                              <a href="Controlador.php?accion=tipofaltas_actualizar&id=' . $d->codtipofalta . '" class="btn btn-outline-success action-btn" title="Editar">
                                  <i class="bi bi-pencil"></i>
                              </a>
                              <a href="Controlador.php?accion=tipofaltas_eliminar&id=' . $d->codtipofalta . '" class="btn btn-outline-danger action-btn" title="Eliminar">
      ...
      <div class="card-footer bg-white py-4">
        <nav aria-label="Navegación de páginas">
          <ul class="pagination justify-content-center mb-0 gap-2">
            ...
            $anteriorLink = ($paginaActual > 1) ? 'Controlador.php?accion=modulo_tipofaltas&pagina=' . (($anterior - 1) * 10) : '#';
            ...
            for ($j = 1; $j <= $totalPaginas; $j++) {
              ...
              echo '<li class="page-item ' . $activeClass . '">
                      <a class="page-link pagination-link" href="Controlador.php?accion=modulo_tipofaltas&pagina=' . $c . '">' . $j . '</a>
                    </li>';
            }

            // Siguiente
            $siguiente = min($totalPaginas, $paginaActual + 1);
            $siguienteLink = ($paginaActual < $totalPaginas) ? 'Controlador.php?accion=modulo_tipofaltas&pagina=' . (($siguiente - 1) * 10) : '#';
