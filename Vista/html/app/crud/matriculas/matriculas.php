        ...
        <div class="container-fluid px-4">
            <div class="card shadow-lg border-0 rounded-4 overflow-hidden">
                ...
                <div
                    ...
                    <div>
                        <h2 class="color_icon fw-bold mb-1">
                            <i class="bi bi-people-fill me-2 color_icon"></i>Lista de Matrículas
                        </h2>
                        <p class="text-muted mb-0 small">Administra todas las matrículas registradas en el sistema</p>
                    </div>
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-3 mt-md-0">
                        ...
                        <a href="Controlador.php?accion=matriculas_insertar"
                            class="btn btn-primary rounded-pill px-4 btn_app">
                            <i class="bi bi-plus-circle me-1"></i> Agregar
                        </a>
                        <a href="Controlador.php?accion=matriculas_pdf"
              ...
              while ($d = $datos->fetch_object()) {
                  $data = '
                  <tr>
                      <td class="ps-4 fw-semibold text-dark">' . $d->codmatricula . '</td>
                      <td>' . $d->estidpersona . '</td>
                      <td class="text-capitalize">' . $d->pernombre . '</td>
                      <td class="text-capitalize">' . $d->perapellido . '</td>
                      <td>' . $d->matnumero . '</td>
                      <td>' . $d->matfecha . '</td>
                      <td>' . $d->matanovigencia . '</td>
                      <td>' . $d->sednombre . '</td>
                      <td>' . $d->granombre . '</td>
                      <td>' . $d->curnombre . '</td>
                      <td class="text-center">
                          <div class="d-flex justify-content-center gap-2">
                              <a href="Controlador.php?accion=matriculas_consultar&id=' . $d->codmatricula . '" class="btn btn-outline-primary action-btn" title="Ver">
                                  <i class="bi bi-eye"></i>
                              </a>
                              <a href="Controlador.php?accion=matriculas_actualizar&id=' . $d->codmatricula . '" class="btn btn-outline-success action-btn" title="Editar">
                                  <i class="bi bi-pencil"></i>
                              </a>
                              <a href="Controlador.php?accion=matriculas_eliminar&id=' . $d->codmatricula . '" class="btn btn-outline-danger action-btn" title="Eliminar">
            ...
            $anteriorLink = ($paginaActual > 1) ? 'Controlador.php?accion=modulo_matriculas&pagina=' . (($anterior - 1) * 10) : '#';
            ...
            for ($j = 1; $j <= $totalPaginas; $j++) {
                ...
                echo '<li class="page-item ' . $activeClass . '">
                        <a class="page-link pagination-link" href="Controlador.php?accion=modulo_matriculas&pagina=' . $c . '">' . $j . '</a>
                      </li>';
            }

            // Página siguiente
            $siguiente = min($totalPaginas, $paginaActual + 1);
            $siguienteLink = ($paginaActual < $totalPaginas) ? 'Controlador.php?accion=modulo_matriculas&pagina=' . (($siguiente - 1) * 10) : '#';
